...
static char FileName[32] = "data.txt";
static char *SD_File;

// XXX u8 DestinationAddress[DATA_TXT_SIZE] __attribute__ ((aligned(32)));
u8* DestinationAddress = NULL;
// XXX u8 SourceAddress[100*1024*1024] __attribute__ ((aligned(32)));
...
/*
 * Loads the file data.txt into memory.
...
u8* sdcard_load(void)
{
    ...
    u32 BuffCnt;
    BYTE work[FF_MAX_SS];
    u32 FileSize = DATA_TXT_SIZE;    // size of data.txt
    ...
    */
    ...
    if (Res != FR_OK) {
        xil_printf("ERROR: problem with f_mount()\n\r");
        return NULL;
    }
    ...
    */
    ...
    if (Res) {
        xil_printf("ERROR: problem with f_open()\n\r");
        return NULL;
    }
    ...
    Res = f_lseek(&fil, 0);
    if (Res) {
        xil_printf("ERROR: problem with f_lseek()\n\r");
        return NULL;
    }
    ...
    */
    ...
    // malloc space for the array of book_info
    DestinationAddress = malloc(DATA_TXT_SIZE*sizeof(char));
    ...
    Res = f_read(&fil, (void*)DestinationAddress, FileSize,
            &NumBytesRead);
    if (Res) {
        xil_printf("ERROR: problem with f_read()\n\r");
        return NULL;
    }
    ...
    */
    ...
    Res = f_close(&fil);
    if (Res) {
        xil_printf("ERROR: problem with f_close()\n\r");
        return NULL;
    }

    return (u8*)DestinationAddress;
}
